...
```powershell
uv run python 2_生產線/run_ocr_pipeline.py data/sources/2023p2.pdf `
...
valid failure audit at output/figure_pipeline/2015p2/q018-b2: the model
...
`output/figure_pipeline/2015p2/q018-b2-reviewed`:
...
proposal at `output/figure_pipeline/2015p2/q018-b3`. Qwen found all
...
`output/figure_pipeline/2015p2/q018-b3-reviewed` with natural order
...
`output/figure_pipeline/benchmark/benchmark-manifest.json` and
...
`3.分析結果/reports/figure_pipeline/b3_visible_label_benchmark_2026-08-05.md`
and `output/figure_pipeline/benchmark/b3-prompt-comparison.json`.
...
`output/figure_pipeline/benchmark/benchmark-manifest-v2.json`.
...
Implementation:
`src/figure_pipeline/sequence_order_metrics.py`,
`sequence_order_evaluator.py`, `sequence_order_cli.py`, and
`scripts/evaluate_b3_sequence_order.py`. Definition and report:
