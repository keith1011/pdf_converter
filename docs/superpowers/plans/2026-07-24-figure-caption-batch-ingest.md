...
**Goal:** Deliver figure crop+Qwen caption into pageir/DONE/Qdrant (`pageir_v2`) and a batch CLI that stages→publishes→ingests local `output/` docs.
...
**Tech Stack:** Existing `ocr_pipeline` + `homelab/ingest` (DONE schema 1), pytest, `uv run`, Qwen2.5-VL via `VlmClient.generate`, Qdrant writer on A.

**Spec:** `docs/superpowers/specs/2026-07-24-trunk-qwen-ingest-contract-design.md`
...
| Path | Responsibility |
|------|----------------|
| `src/ocr_pipeline/models.py` | `SegmentKind.FIGURE`; `ContentSegment.crop_relpath` |
| `src/ocr_pipeline/content_first.py` | Serialize `crop_relpath`; render `(圖: …)`; merge figure segments in finalize |
| `src/ocr_pipeline/prompts.py` | `FIGURE_CAPTION_PROMPT` |
| `src/ocr_pipeline/figure_export.py` | Crop→`figures/`, caption via VLM, build figure `ContentSegment`s |
| `src/ocr_pipeline/routers.py` | Always crop FIGURE; `skip_figures` only skips draft OCR stitch |
| `src/ocr_pipeline/pipeline.py` | Call figure export; pass figures into finalize |
| `src/ocr_pipeline/factory.py` | Wire `extract_figures` from config |
| `src/ocr_pipeline/job_stage.py` | Build flat staging dir: `{doc}.txt/.tex/.pageir.json` + `figures/*` |
| `src/ocr_pipeline/batch_export.py` | CLI: docs → stage → publish → ingest |
| `config/ocr_pipeline.yaml` | `extract_figures: true`; clarify `skip_figures` |
| `homelab/ingest/publish.py` | Publish from staged dir including `figures/` |
| `homelab/ingest/done.py` | If `figures/` files exist on disk, require them in artifacts |
| `homelab/ingest/ingest.py` | `CHUNK_VERSION=pageir_v2`; payload `crop_path` for figure kind |
| `tests/test_page_ir_models.py` | FIGURE + crop_relpath |
| `tests/test_content_first.py` | Render + JSON round-trip for figures |
| `tests/test_figure_export.py` | Caption/crop/skip-on-error (mocked VLM) |
| `tests/test_skip_figures_router.py` | Update: skip_figures still crops |
| `tests/test_job_stage.py` | Staging layout |
| `tests/test_done_schema.py` | Nested `figures/` artifact |
| `tests/test_ingest_figures.py` | Mocked load_segments / payload fields |
| `tests/test_batch_export.py` | Call order + continue-on-error |
...
| Batch CLI from `output/` with continue-on-error | Task 6 |
