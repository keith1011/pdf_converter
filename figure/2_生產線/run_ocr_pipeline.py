...
Usage:
  uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/123.pdf
  uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/123.pdf --limit 1
  uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/123.pdf --check-compile
"""
...
def build_parser(pipe_cfg: dict | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--reuse-layout",
        action="store_true",
        help="Skip Surya; load 1_收集資料/data/pdf_pages/<stem>/layout.json (implies image reuse)",
    )
    parser.add_argument(
        "--dse-mcq",
        action="store_true",
    ...
    parser.add_argument(
        "--text-engine",
        choices=("vlm", "ppocr", "paddleocr_vl"),
        default=None,
        help="Override the configured Stage2 text engine for this run only",
    )
    parser.add_argument(
        "--crop-dir",
        type=Path,
