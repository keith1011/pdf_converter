...
| Path | Responsibility |
|------|----------------|
| `2_生產線/src/ocr_pipeline/engines/base.py` | Protocols + `EngineError` |
| `2_生產線/src/ocr_pipeline/engines/timing.py` | Stage timer helper (logged, not scored) |
| `2_生產線/src/ocr_pipeline/engines/surya_layout.py` | `LayoutAnalyzer` → `LayoutEngine` |
| `2_生產線/src/ocr_pipeline/engines/vlm_text.py` | Qwen/GLM `TextEngine` |
| `2_生產線/src/ocr_pipeline/engines/vlm_formula.py` | Qwen/GLM `FormulaEngine` |
| `2_生產線/src/ocr_pipeline/engines/ppocr_text.py` | PP-OCR Traditional text engine |
| `2_生產線/src/ocr_pipeline/engines/got_formula.py` | GOT-OCR 2.0 formula engine |
| `2_生產線/src/ocr_pipeline/engines/doclayout_yolo.py` | DocLayout-YOLO layout engine |
| `2_生產線/src/ocr_pipeline/engines/mineru_layout.py` | MinerU layout engine |
| `2_生產線/src/ocr_pipeline/engines/unimernet_formula.py` | UniMERNet formula engine |
| `2_生產線/src/ocr_pipeline/engines/__init__.py` | Public exports |
| `2_生產線/src/ocr_pipeline/factory.py` | Build engines from config |
| `2_生產線/src/ocr_pipeline/routers.py` | Route via `TextEngine` / `FormulaEngine` |
| `2_生產線/src/ocr_pipeline/pipeline.py` | Call `LayoutEngine`; optional skip polish; timing; output tag |
| `2_生產線/config/ocr_pipeline.yaml` | `engines:` block + `skip_polish` + `output_tag` |
| `2_生產線/tests/test_engines_contract.py` | Fake engines + protocol tests |
| `2_生產線/tests/test_engines_pipeline_smoke.py` | skip_polish + engine duck-typing smoke |
| `3.分析結果/docs/superpowers/evals/p-ocr-branch-scorecard.md` | Human D scorecard + timing log template |
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/__init__.py`
- Create: `2_生產線/src/ocr_pipeline/engines/base.py`
- Test: `2_生產線/tests/test_engines_contract.py`
...
```python
# 2_生產線/tests/test_engines_contract.py
...
Run: `$env:PYTHONPATH="src"; .\.venv\Scripts\python.exe -m pytest 2_生產線/tests/test_engines_contract.py -q`  
Expected: FAIL (`ocr_pipeline.engines.base` missing)

- [ ] **Step 3: Write minimal implementation**

```python
# 2_生產線/src/ocr_pipeline/engines/base.py
from __future__ import annotations
...
```python
# 2_生產線/src/ocr_pipeline/engines/__init__.py
...
```bash
git add 2_生產線/src/ocr_pipeline/engines 2_生產線/tests/test_engines_contract.py
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/timing.py`
- Modify: `2_生產線/tests/test_engines_contract.py`
...
```python
# 2_生產線/src/ocr_pipeline/engines/timing.py
from __future__ import annotations
...
```bash
git add 2_生產線/src/ocr_pipeline/engines/timing.py 2_生產線/tests/test_engines_contract.py
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/surya_layout.py`
- Create: `2_生產線/src/ocr_pipeline/engines/vlm_text.py`
- Create: `2_生產線/src/ocr_pipeline/engines/vlm_formula.py`
- Test: extend `2_生產線/tests/test_engines_contract.py`
...
```python
# 2_生產線/src/ocr_pipeline/engines/surya_layout.py
from __future__ import annotations
...
```python
# 2_生產線/src/ocr_pipeline/engines/vlm_text.py
from __future__ import annotations
...
```python
# 2_生產線/src/ocr_pipeline/engines/vlm_formula.py
from __future__ import annotations
...
- Modify: `2_生產線/run_ocr_pipeline.py` (optional `--output-tag` / `--skip-polish`)
- Test: `2_生產線/tests/test_engines_pipeline_smoke.py`
...
```python
# 2_生產線/tests/test_engines_pipeline_smoke.py
from pathlib import Path
...
```powershell
$env:PYTHONPATH="src"
.\.venv\Scripts\python.exe -m pytest 2_生產線/tests/test_engines_contract.py 2_生產線/tests/test_engines_pipeline_smoke.py 2_生產線/tests/test_content_first_pipeline.py 2_生產線/tests/test_polish_strategy.py -q
```
...
```bash
git add 2_生產線/config/ocr_pipeline.yaml
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/ppocr_text.py`
- Create: `requirements-ppocr.txt` (`paddleocr` + paddlepaddle CUDA pin chosen at install time)
- Test: `2_生產線/tests/test_ppocr_text_engine.py` (mock OCR)
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/doclayout_yolo.py`
- Create: `2_生產線/src/ocr_pipeline/engines/got_formula.py`
...
```powershell
$env:PYTHONPATH="src"
.\.venv\Scripts\python.exe 2_生產線/run_ocr_pipeline.py 1_收集資料\data\sources\123.pdf --limit 1 --reuse-images --check-compile
```
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/engines/mineru_layout.py`
- Create: `2_生產線/src/ocr_pipeline/engines/unimernet_formula.py`
...
**Files:**
- Modify: `2_生產線/src/ocr_pipeline/glm_client.py` docstring → deprecated
- Modify: `2_生產線/config/ocr_pipeline.yaml` `glm:` comment
- Create: `2_生產線/_script/cleanup_glm_weights.ps1` (dry-run by default; `-Confirm` deletes HF cache for `zai-org/GLM-4.6V-Flash` only)
...
**Files:**
- Create: `3.分析結果/docs/superpowers/evals/p-ocr-branch-scorecard.md`
