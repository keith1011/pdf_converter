...
| Path | Responsibility |
|------|----------------|
| `2_生產線/src/ocr_pipeline/nup_types.py` | `NupClass`, `NupPanel`, `NupDecision` dataclasses |
| `2_生產線/src/ocr_pipeline/nup_crop.py` | Fixed midline / 2×2 pixel crops + write panel PNGs |
| `2_生產線/src/ocr_pipeline/nup_classify.py` | Projection-valley classifier → class + confidence |
| `2_生產線/src/ocr_pipeline/nup_router.py` | Threshold gate; orchestrate classify→crop or fallback; write `nup.json` |
| `2_生產線/src/ocr_pipeline/nup_merge.py` | Remap panel blocks to page coords; order by `version_id` |
| `2_生產線/src/ocr_pipeline/nup_label.py` | Optional semantic label heuristics (EN/ZH) |
| `2_生產線/src/ocr_pipeline/models.py` | `ContentSegment.version_id`; optional `PageResult.version_id` |
| `2_生產線/src/ocr_pipeline/content_first.py` | Serialize `version_id`; preserve through integrity |
| `2_生產線/src/ocr_pipeline/pipeline.py` | Call N-up gate before `_analyze`; merge panels into one `PageResult` |
| `2_生產線/src/ocr_pipeline/factory.py` | Wire `nup:` config |
| `2_生產線/config/ocr_pipeline.yaml` | `nup.enabled`, `confidence_threshold`, `margin_px` |
| `2_生產線/tests/test_nup_crop.py` | Crop geometry |
| `2_生產線/tests/test_nup_classify.py` | Synthetic 1/2/4 pages |
| `2_生產線/tests/test_nup_router.py` | Fallback vs split |
| `2_生產線/tests/test_nup_merge.py` | Version order + bbox remap |
| `2_生產線/tests/test_nup_pageir.py` | `version_id` in pageir JSON |
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/nup_types.py`
- Create: `2_生產線/src/ocr_pipeline/nup_crop.py`
- Test: `2_生產線/tests/test_nup_crop.py`
...
Run: `uv run pytest 2_生產線/tests/test_nup_crop.py -q`  
Expected: FAIL (import / not found)

- [ ] **Step 3: Minimal implementation**

```python
# 2_生產線/src/ocr_pipeline/nup_types.py
...
```python
# 2_生產線/src/ocr_pipeline/nup_crop.py
from __future__ import annotations
from pathlib import Path
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/nup_classify.py`
- Test: `2_生產線/tests/test_nup_classify.py`
...
```python
# 2_生產線/tests/test_nup_classify.py
from pathlib import Path
...
Run: `uv run pytest 2_生產線/tests/test_nup_classify.py -q`  
Expected: FAIL
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/nup_router.py`
- Test: `2_生產線/tests/test_nup_router.py`
...
```python
# 2_生產線/tests/test_nup_router.py
from pathlib import Path
...
- [ ] **Step 2–4:** Implement router + JSON round-trip; run `uv run pytest 2_生產線/tests/test_nup_router.py -q` → PASS
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/nup_merge.py`
- Test: `2_生產線/tests/test_nup_merge.py`
...
- [ ] **Step 2–4:** Implement; `uv run pytest 2_生產線/tests/test_nup_merge.py -q` → PASS
...
**Files:**
- Modify: `2_生產線/config/ocr_pipeline.yaml` (add `nup:` block)
- Modify: `2_生產線/src/ocr_pipeline/factory.py` (read nup settings onto pipeline)
- Modify: `2_生產線/src/ocr_pipeline/pipeline.py` (Stage1 loop)
- Test: `2_生產線/tests/test_nup_pipeline_gate.py` (mock layout analyze)
...
```python
# 2_生產線/tests/test_nup_pipeline_gate.py
...
**Files:**
- Modify: `2_生產線/src/ocr_pipeline/models.py` — `ContentSegment.version_id: str | None = None`
- Modify: `2_生產線/src/ocr_pipeline/segmenter.py` / `content_first.py` — propagate `version_id` from block.meta when segmenting; JSON field `version_id`
- Create: `2_生產線/src/ocr_pipeline/nup_label.py` — `maybe_label_panels(decision: NupDecision, drafts: dict[str,str]) -> NupDecision`
- Test: `2_生產線/tests/test_nup_pageir.py`
- Test: `2_生產線/tests/test_nup_label.py`
...
- [ ] **Step 2–4:** Implement; run `uv run pytest 2_生產線/tests/test_nup_pageir.py 2_生產線/tests/test_nup_label.py 2_生產線/tests/test_content_first.py -q` → PASS
...
**Files:**
- Test: `2_生產線/tests/test_nup_interleave_guard.py`
- Optional fixture PNGs under `2_生產線/tests/fixtures/nup/` (synthetic L/R with distinct markers `Q16` / `Q19` painted as text or as separate ink blobs tagged in meta via mock analyze)
...
`uv run pytest 2_生產線/tests/test_nup_*.py 2_生產線/tests/test_reading_order.py -q`  
Expected: PASS
