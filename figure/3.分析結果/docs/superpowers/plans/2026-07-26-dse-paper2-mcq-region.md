...
**Spec:** `3.分析結果/docs/superpowers/specs/2026-07-26-dse-paper2-mcq-region-design.md`

## Global Constraints

- Ideal: **1 MCQ = 1 box** (stem + figure + A–D).
- v1 profile: **MATH CP P2 only** (`2_生產線/config/profiles/math_cp_p2.yaml`); `dual_column: false`.
...
| Path | Responsibility |
|------|----------------|
| `2_生產線/src/ocr_pipeline/dse_mcq_types.py` | `OcrLine`, `LineTag`, `McqRegion`, `McqAnchors` dataclasses |
| `2_生產線/src/ocr_pipeline/dse_mcq_profile.py` | Load/validate subject profile YAML → `McqProfile` |
| `2_生產線/src/ocr_pipeline/dse_mcq_region.py` | Tag lines + pair openers→A–D spans → list[`McqRegion`] |
| `2_生產線/src/ocr_pipeline/dse_mcq_layout.py` | Regions → crops + `LayoutBlock`s + `save_layout_artifact` |
| `2_生產線/src/ocr_pipeline/dse_mcq_ocr.py` | Optional `LightOcr` protocol + JSON lines loader (no hard paddle dep) |
| `2_生產線/config/profiles/math_cp_p2.yaml` | MATH CP P2 patterns / margins / `min_option_hits` |
| `2_生產線/_script/dse_mcq_regions.py` | CLI: page PNG + lines JSON (or OCR if available) → `layout.json` |
| `2_生產線/tests/test_dse_mcq_profile.py` | Profile load + defaults |
| `2_生產線/tests/test_dse_mcq_region.py` | Opener/option pairing; reject mid-stem `1.` |
| `2_生產線/tests/test_dse_mcq_layout.py` | Crops + layout.json roundtrip via `load_layout_artifact` |
...
3. **Separate script** `2_生產線/_script/dse_mcq_regions.py` then A `--reuse-layout` — no `--layout-engine` pipeline flag in v1.
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/dse_mcq_types.py`
- Create: `2_生產線/src/ocr_pipeline/dse_mcq_profile.py`
- Create: `2_生產線/config/profiles/math_cp_p2.yaml`
- Test: `2_生產線/tests/test_dse_mcq_profile.py`
...
- Produces:
  ...
  - `default_math_cp_p2_path() -> Path` (repo `2_生產線/config/profiles/math_cp_p2.yaml`)

- [x] **Step 1: Write the failing test**

```python
# 2_生產線/tests/test_dse_mcq_profile.py
...
Run: `uv run pytest 2_生產線/tests/test_dse_mcq_profile.py -q`
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/dse_mcq_region.py`
- Test: `2_生產線/tests/test_dse_mcq_region.py`
...
```python
# 2_生產線/tests/test_dse_mcq_region.py
from ocr_pipeline.dse_mcq_profile import default_math_cp_p2_path, load_mcq_profile
...
Run: `uv run pytest 2_生產線/tests/test_dse_mcq_region.py -q`
Expected: FAIL
...
Run: `uv run pytest 2_生產線/tests/test_dse_mcq_region.py 2_生產線/tests/test_dse_mcq_profile.py -q`
Expected: PASS
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/dse_mcq_layout.py`
- Test: `2_生產線/tests/test_dse_mcq_layout.py`
...
```python
# 2_生產線/tests/test_dse_mcq_layout.py
from pathlib import Path
...
Run: `uv run pytest 2_生產線/tests/test_dse_mcq_layout.py -q`
Expected: FAIL
...
Run: `uv run pytest 2_生產線/tests/test_dse_mcq_layout.py 2_生產線/tests/test_dse_mcq_region.py -q`
Expected: PASS
...
**Files:**
- Create: `2_生產線/src/ocr_pipeline/dse_mcq_ocr.py`
- Create: `2_生產線/_script/dse_mcq_regions.py`
- Test: extend `2_生產線/tests/test_dse_mcq_layout.py` or `2_生產線/tests/test_dse_mcq_ocr.py`
...
- CLI:
  ```
  uv run python 2_生產線/_script/dse_mcq_regions.py \
    --image path/to/page.png \
    --lines path/to/lines.json \
    --profile 2_生產線/config/profiles/math_cp_p2.yaml \
    --out-dir 1_收集資料/data/pdf_pages/<stem>/
  ```
...
```python
# 2_生產線/tests/test_dse_mcq_ocr.py
import json
from pathlib import Path
...
- [x] Run: `uv run pytest 2_生產線/tests/test_dse_mcq_profile.py 2_生產線/tests/test_dse_mcq_region.py 2_生產線/tests/test_dse_mcq_layout.py 2_生產線/tests/test_dse_mcq_ocr.py -q`
- [x] Run: `uv run ruff check` + `uv run ruff format` on touched `2_生產線/src/ocr_pipeline/dse_mcq_*.py`, `2_生產線/_script/dse_mcq_regions.py`, tests
