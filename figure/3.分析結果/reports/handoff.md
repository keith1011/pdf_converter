...
Do **not** invent state. Prefer filesystem planning docs over chat memory. Never paste API keys (Qdrant reader key in `~/.cursor/mcp.json`; writer keys in B `~/2_生產線/homelab/.env` only).
...
Contract: `2_生產線/homelab/DATA_PLANE.md` + design `DONE.json` schema.
...
- Scorecard: `3.分析結果/docs/superpowers/evals/p-ocr-branch-scorecard.md`
...
| Compile path (offline) | Re-finalize `3.分析結果/output/123.tex` → **COMPILE_OK**, PDF ~101KB (2026-07-23). Backup: `3.分析結果/output/123.tex.bak_tabular` |
...
- `2_生產線/src/ocr_pipeline/vlm_client.py` — Qwen greedy `build_generation_kwargs`
- `2_生產線/src/ocr_pipeline/layout.py` — `_stop_surya_docker_vlms` + GPU headroom wait in `release()`
- `2_生產線/src/ocr_pipeline/segmenter.py`, `prompts.py` (TABLE_ROUTER linear), `formula_integrity.py`, content-first related
- `2_生產線/config/ocr_pipeline.yaml` — `temperature: 0.0`
- Tests: `2_生產線/tests/test_vlm_greedy_decode.py`, `2_生產線/tests/test_layout_release.py`, segmenter/content_first/integrity updates
- Planning: `task_plan.md`, `findings.md`, `progress.md`
- Homelab: `2_生產線/homelab/` (compose, ingest, DONE schema, docs)
...
```powershell
$env:PYTHONPATH = "src"
.\.venv\Scripts\python.exe 2_生產線/run_ocr_pipeline.py 1_收集資料\data\sources\123.pdf --check-compile
...
| Path | Role |
|------|------|
| `2_生產線/run_ocr_pipeline.py` | CLI entry |
| `2_生產線/config/ocr_pipeline.yaml` | VLM/layout knobs (`temperature: 0.0`) |
| `2_生產線/src/ocr_pipeline/layout.py` | Surya + **`release()` docker stop** |
| `2_生產線/src/ocr_pipeline/vlm_client.py` | Qwen client; **`build_generation_kwargs` greedy** |
| `2_生產線/src/ocr_pipeline/routers.py` | Stage2 routing |
| `2_生產線/src/ocr_pipeline/prompts.py` | **`TABLE_ROUTER_PROMPT` = linear** (no tabular/Markdown tables) |
| `2_生產線/src/ocr_pipeline/segmenter.py` | Stitch → PageIR; tabular/CJK/`\caption` defenses |
| `2_生產線/src/ocr_pipeline/formula_integrity.py` | Math body checks; flags tabular chrome in math |
| `2_生產線/src/ocr_pipeline/content_first.py` | `finalize_content_first` → txt/tex/pageir |
| `2_生產線/src/ocr_pipeline/compile_check.py` | Ship 1.5 compile gate |
| `2_生產線/homelab/` | B data plane + `ingest/` publish/DONE |
| `3.分析結果/output/123.*` | Current golden artifacts; `.tex.bak_tabular` pre-repair backup |
...
| Ingest code | `2_生產線/homelab/ingest/` (`done.py`, `publish.py`, `ingest.py`) |
| Contract | `2_生產線/homelab/DATA_PLANE.md`, `2_生產線/homelab/README.md` |
| Embedding | On **A**: `nomic-ai/nomic-embed-text-v1.5` (768-d) |
| Collections | MCP `memories` ≠ 題庫 collection (`exam_segments_v1`) |

Keys: reader in `~/.cursor/mcp.json`; writer/reader pair on B `~/2_生產線/homelab/.env`. **Never commit or paste values.**
...
.\.venv\Scripts\python.exe -m pytest 2_生產線/tests/test_vlm_greedy_decode.py 2_生產線/tests/test_layout_release.py -q
.\.venv\Scripts\python.exe -m pytest 2_生產線/tests/test_segmenter.py 2_生產線/tests/test_content_first.py 2_生產線/tests/test_content_first_prompts.py 2_生產線/tests/test_formula_integrity.py -q

# DONE schema (homelab)
.\.venv\Scripts\python.exe -m pytest 2_生產線/tests/test_done_schema.py -q

# Compile gate on existing draft (offline)
.\.venv\Scripts\python.exe -c "from pathlib import Path; from ocr_pipeline.compile_check import compile_tex; r=compile_tex(Path('3.分析結果/output/123.tex')); print(r.ok, r.message, r.pdf_path)"
