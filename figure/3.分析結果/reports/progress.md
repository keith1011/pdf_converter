...
## 2026-07-31 — OpenCode export zip
- Built `pdf-scaner-opencode.zip` (+ Desktop copy): 2_生產線/src/2_生產線/config/2_生產線/tests/2_生產線/_script/homelab + synthesized docs.
- Excluded `.cursor`, `AGENTS.md`, `CLAUDE.md`, SKILL/MCP handbooks, venvs, large data/output.
- Entry: `OPENCODE_START_HERE.md`; AGENTS generation prompt: `3.分析結果/docs/GENERATE_AGENTS_PROMPT.md`.
- Staging kept under `_opencode_export/pdf-scaner-opencode/`.

## 2026-07-28 14:28 — Wrote 3.分析結果/reports/handoff_ocr.md for Codex
...
- Exported Desktop + opened `3.分析結果/output/2015p2.mcq.txt`.
...
- Command: `uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/2015p2.pdf --reuse-layout --output-tag mcq`
...
- **Timing** (`3.分析結果/output/2015p2.mcq.timing.txt`): wall **4502s (1h15m02s)**; route=701s · polish=3796s · total=4497s
...
- Command: `uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/2015p2.pdf --reuse-layout --output-tag mcq`
- Expect longer Stage3 (per-question polish). Timing → `3.分析結果/output/2015p2.mcq.timing.txt`
...
- Deleted old `3.分析結果/output/*` document artifacts; kept `dse_mcq_layout_*` + smoke + `.gitkeep`.
- Re-run: `--reuse-layout --output-tag mcq` → **exit 0**
- **Timing** (`3.分析結果/output/2015p2.mcq.timing.txt`):
...
- Artifacts: `3.分析結果/output/2015p2.mcq.{tex,txt,pageir.json,quality.json,run.log,timing.txt}`
...
- Fix: skip VLM on blank draft; reject output that looks like math-rules echo; strip leaked block from `3.分析結果/output/2015p2.mcq.{tex,txt}`.
- Tests: `test_polish_extract` 5 passed.

## 2026-07-27 09:40 — Start VLM on 2015p2 MCQ layout
- Command: `uv run python 2_生產線/run_ocr_pipeline.py 1_收集資料/data/sources/2015p2.pdf --reuse-layout --output-tag mcq`
- Layout: `1_收集資料/data/pdf_pages/2015p2/layout.json` (15 pages, 45 TEXT blocks, source=`dse_mcq_region:math_cp_p2`)
- GPU preflight: ~2.0/12.3 GiB used before load
- **Done** exit 0 in ~784s (route 311s + polish 466s). Artifacts: `3.分析結果/output/2015p2.mcq.{tex,txt,pageir.json,quality.json}`
...
- Unit tests 12 passed. Overlays refreshed under `3.分析結果/output/dse_mcq_layout_2015p2/`.

## 2026-07-27 08:40 — Full 2015p2 MCQ layout
- Ran `2_生產線/_script/dse_mcq_layout_doc.py` on all 15 pages (~70s with RapidOCR).
- Wrote `1_收集資料/data/pdf_pages/2015p2/layout.json` (48 Q-boxes); overlays in `3.分析結果/output/dse_mcq_layout_2015p2/`.
...
- Overlay: `3.分析結果/output/dse_mcq_smoke_2015p2_p4/overlay.png`; `test_dse_mcq_region` 6 passed.
...
- Wrote `3.分析結果/docs/superpowers/specs/2026-07-26-dse-paper2-mcq-region-design.md` (Approved).
...
- Full 8p exit 0 (~843s): `3.分析結果/output/2014-DSE-MATH-CP-2.nup-full.*`; GPU peak ~9GB.
...
- Artifacts: `3.分析結果/output/2014-DSE-MATH-CP-2.nup-smoke.*`; `nup/page_00{1,2,3}/nup.json`.
...
- Plan: `3.分析結果/docs/superpowers/plans/2026-07-25-nup-classifier-crop.md` (Tasks 1–7, TDD)
...
- Review copy refreshed: `3.分析結果/output/_review/2014-DSE-MATH-CP-2/`.
...
- Killed resume + `run_ocr_pipeline` (2014) PIDs; removed `3.分析結果/output/.ocr_pipeline.lock`.
...
- PDFs: `2012p2`, `2013p2`, `2014-DSE-MATH-CP-2`, `2015p2`, `2016p2` in `1_收集資料/data/sources/`.
...
- User will drop ~5 × ~20pp PDFs with figures into `1_收集資料/data/sources/`.
...
- Forced figure path: `export_figures` → PNG + caption (`3.分析結果/output/_smoke_fig/figures/p001_bFIG.png`, 9KB).
...
- Verification: `uv run pytest 2_生產線/tests/test_batch_export.py -q` → **2 passed**.
...
- Implemented `2_生產線/src/ocr_pipeline/batch_export.py`: stage→publish→ingest, preflight, fail-continue, lazy monkeypatchable homelab imports.
...
- Pipeline exports captions to `3.分析結果/output/<artifact_source>/figures`, groups them by page, and finalizes them into txt/tex/PageIR.
...
- Spec locked: `3.分析結果/docs/superpowers/specs/2026-07-24-trunk-qwen-ingest-contract-design.md`
- Plan written: `3.分析結果/docs/superpowers/plans/2026-07-24-figure-caption-batch-ingest.md` (Tasks 1–7)
...
- Updated `2_生產線/config/ocr_pipeline.yaml`, `task_plan.md`, `findings.md`, design spec trunk layout note.
...
- Commit `db664cf`: homelab Wave 1 2_生產線/_script/ingest/DATA_PLANE, Cursor rules, handbooks, `.python-version`, `requires-python >=3.12,<3.13`
...
- Ran ruff check/format on `2_生產線/src/ocr_pipeline/engines` + related tests; fixed I001 import order; formatted 6 files.
...
- **Hard gate:** run on B (or password-SSH from A):
  ...
  `bash /data/pdf-scaner/backups/wave1-2_生產線/_script/wave1-on-b.sh`

## 2026-07-23 21:10 — Homelab Wave 1 (partial; waiting on B)
- Scripts added under `2_生產線/homelab/2_生產線/_script/`: ufw, backup RP, restore drill, key apply, reader neg-test, wave1-on-b, A finish helpers.
- Samba sync: `Z:\backups\wave1-2_生產線\_script\`, `Z:\backups\ssh-bootstrap\pc-a.pub`.
...
- **User action on B:** `bash /data/pdf-scaner/backups/ssh-bootstrap/install-pc-a-key.sh` then `bash /data/pdf-scaner/backups/wave1-2_生產線/_script/wave1-on-b.sh`
- After `WAVE1_B_DONE`: run `2_生產線/homelab/2_生產線/_script/wave1-finish-on-a.ps1` on A.
...
- Main `2_生產線/config/ocr_pipeline.yaml` restored to surya/vlm defaults afterward.

## 2026-07-23 — Task 10 P-ocr branch comparison scorecard
- Added `3.分析結果/docs/superpowers/evals/p-ocr-branch-scorecard.md` for `qwen-vl`, `got-ppocr`, and `mineru-ppocr`.
...
- References: spec `3.分析結果/docs/superpowers/specs/2026-07-23-ocr-engine-adapters-design.md`; plan `3.分析結果/docs/superpowers/plans/2026-07-23-ocr-engine-adapters.md`.
...
- Plan committed: `3.分析結果/docs/superpowers/plans/2026-07-23-ocr-engine-adapters.md` (`79dd2f2`)
...
- Offline re-finalize `3.分析結果/output/123.tex` (backup `.tex.bak_tabular`) → **COMPILE_OK**, PDF ~101KB
...
- Added `2_生產線/homelab/ingest/` (`done.py`, `publish.py`, `ingest.py`) + `2_生產線/tests/test_done_schema.py` (4 passed)
...
- Updated `2_生產線/homelab/README.md`

## 2026-07-22 21:26 — Homelab Wave 0 start
- OS decision: **Ubuntu Server 24.04 LTS** on PC-B
- Added repo `2_生產線/homelab/`: README checklist, `docker-compose.yml` (Qdrant v1.13.2), `.env.example`, `DATA_PLANE.md`
...
- Command: `2_生產線/run_ocr_pipeline.py 1_收集資料\data\sources\123.pdf --check-compile`
...
| `2_生產線/run_ocr_pipeline.py ... --limit 1 --reuse-images --check-compile` | **exit 0** |
...
3. Homelab follow-ups (still uncommitted under `2_生產線/homelab/`)
...
- `uv run pytest` quality+ingest gate suites green; ruff --fix on touched 2_生產線/tests/batch_export
...
- Full resume exit 0: `TIMING: route=400s polish=389s total=802s` → `3.分析結果/output/2014-DSE-MATH-CP-2.nup-v2.*`
...
3. Light tooling: `pyproject.toml` (ruff only) + `requirements-dev.txt`; `uv pip install -p .venv ruff`; `ruff check 2_生產線/src/ocr_pipeline …` clean; 26 related tests passed
...
- LayoutArtifact: `layout_artifact.py` → `1_收集資料/data/pdf_pages/<stem>/layout.json`; CLI `--reuse-layout`
- Single-instance: `pipeline_lock.py` + `3.分析結果/output/.ocr_pipeline.lock`; CLI `--allow-concurrent`
...
- Excluded: OneDrive phantom M files (empty numstat), `2_生產線/homelab/`, pytest/golden report artifacts, `.cursor/`
...
- Scope: `2_生產線/homelab/ingest/ingest.py` + `2_生產線/tests/test_ingest_figures.py` only
...
- GREEN: `uv run pytest 2_生產線/tests/test_ingest_figures.py -q` → 2 passed
...
- Read `3.分析結果/reports/handoff_ocr.md`, planning files, and user-reviewed `error.txt`.
- Replayed region detection from saved `.lines.json`; no GPU/VLM run.
- Generated 14 affected-page overlays under `3.分析結果/output/missing_question_overlays/`.
- TDD RED command: `uv run python -m pytest 2_生產線/tests/test_dse_mcq_region.py 2_生產線/tests/test_mcq_stage3_sanitize.py -q`
...
## 2026-07-29 - 2015p2 local structured validation smoke run

- Added local-only `parse_stage2_text()` validation with Pydantic v2; the local
  Qwen Transformers Stage2 path remains unchanged. `structured_ocr.enabled` stays
  `false`; no Instructor or external Vision API request was made.
- Rebuilt `1_收集資料/data/pdf_pages/2015p2` from `1_收集資料/data/sources/2015p2.pdf`. The DSE layout
  skips page 1, contains Q1-Q45, and has 0 incomplete boxes; 14 overlays were
  generated for pages 2-15.
- Foreground run `2015p2.mcq-local-validated` completed with 45 questions,
  quality verdict `pass`, local structured validation 45/45 valid, and wall time
  356.3 seconds (`route=353.410s`).

## 2026-07-30 — PaddleOCR-VL 1.6 Stage3 and 2013p2 trial

- Kept Qwen3-VL unchanged and added the isolated PaddleOCR-VL worker path.
  Paddle runtime now loads cuDNN 9.9.0, matching its compiled cuDNN version;
  the main PyTorch/Qwen environment remains on its own cuDNN 9.10.2.
- Fixed DSE crop construction so a short formula line vertically overlapping
  the printed qid is included in the question region. Generic PDF crops are
  unaffected.
- Added Paddle-only deterministic `paddle_sanitize`: Pydantic parsing plus
  exact five-line rendering, with no second VLM inference. HTML image
  references are removed from TXT/TEX; the original crop is retained and
  image-bearing questions are marked `requires_review`.
- Added `--crop-dir` for per-run Stage2 crop isolation. Existing layouts,
  crops, caches, and outputs were not overwritten.
- The first 2013 launcher call hit a 5-second tool timeout after starting
  Stage1. Its partial `1_收集資料/data/pdf_pages/2013p2-paddle-stage3` artifact was
  preserved; RapidOCR completed a valid Q1--Q45 layout.
- Foreground continuation with `--reuse-layout` completed 45 questions in
  **134.089 seconds** (`route=134.054s`, Stage3 `0.010s`), quality verdict
  `pass`. Saved output tag: `paddle-stage3-r2`.
- Post-run local replay with the final parser: **45/45 exact five-line
  renders**, 0 parser failures, 0 remaining HTML references. Fourteen
  image-bearing questions require crop review:
  Q7, Q14--Q20, Q28--Q30, Q32, Q39, and Q41.
