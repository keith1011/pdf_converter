...
- [x] 輸出 14 張缺題頁 overlay 到 `3.分析結果/output/missing_question_overlays/`。
...
- Spec/plan: `3.分析結果/docs/superpowers/specs|plans/2026-07-26-dse-paper2-mcq-region*`
- Layout: `1_收集資料/data/pdf_pages/2015p2/layout.json` (qids 1–45)
...
- Domain table: `.cursor/rules/tool-routing.mdc` (Skill + MCP); handbooks `3.分析結果/docs/SKILL_HANDBOOK.md`, `3.分析結果/docs/MCP_HANDBOOK.md`
...
- [x] Single-instance lock (`3.分析結果/output/.ocr_pipeline.lock`; `--allow-concurrent` escape)
...
- [x] `2_生產線/src/ocr_pipeline/batch_export.py` + `2_生產線/tests/test_batch_export.py`
...
- [x] Create `got-ppocr` branch configuration and commit
  ...
  - [x] GPU smoke limit-1 (2026-07-23): total=33.880s → `3.分析結果/output/123.got-ppocr.tex` (PP-OCR CPU; GOT HF-native)
...
- [x] Focused CPU pytest: 10 passed
  ...
  - [x] GPU smoke limit-1 (2026-07-23): total=39.228s → `3.分析結果/output/123.mineru-ppocr.tex` (dedicated `.venv-mineru312`, transformers 4.57)

### Task 10: P-ocr branch comparison scorecard — complete
- [x] Scorecard: `3.分析結果/docs/superpowers/evals/p-ocr-branch-scorecard.md`
- [x] References: spec `3.分析結果/docs/superpowers/specs/2026-07-23-ocr-engine-adapters-design.md`; plan `3.分析結果/docs/superpowers/plans/2026-07-23-ocr-engine-adapters.md`
...
- [x] Repo scripts: `2_生產線/homelab/2_生產線/_script/` (ufw, backup RP, restore drill, key apply, reader neg-test, wave1-on-b)
...
| LayoutArtifact beside page PNGs | `1_收集資料/data/pdf_pages/<stem>/layout.json`; `--reuse-layout` skips Surya |
...
## Next Action
1. Spot-check overlays in `3.分析結果/output/dse_mcq_layout_2015p2/` (esp. p3/p6/p9/p11/p13)
