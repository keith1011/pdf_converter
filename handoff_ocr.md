...
Golden doc: **`data/sources/2015p2.pdf`** · layout artifact: **`data/pdf_pages/2015p2/layout.json`** (45/45 qids).
...
| Teacher-checkable txt | `output/2015p2.mcq.txt` (also copied to Desktop) |

Artifacts:

```
output/2015p2.mcq.{txt,tex,questions.jsonl,pageir.json,quality.json,timing.txt,run.log}
```
...
- Pre/post comparison overlays:
  `output/missing_question_overlays/` and `output/fixed_question_overlays/`.
...
- Added an opt-in OpenAI-compatible Instructor backend in
  `src/ocr_pipeline/mcq_structured.py`, constructed through `vlm_client.py`.
...
| File | Role |
|------|------|
| `src/ocr_pipeline/dse_mcq_*.py` | Regioner / profile / layout → `layout.json` |
| `src/ocr_pipeline/routers.py` | `question_id` → `MCQ_ROUTER_PROMPT` |
| `src/ocr_pipeline/prompts.py` | `MCQ_ROUTER_PROMPT`, `MCQ_POLISH_PROMPT` |
| `src/ocr_pipeline/assemble.py` | stitch / `split_question_chunks` / `polish_mcq` sanitize |
| `src/ocr_pipeline/pylatex_assist.py` | Unicode→LaTeX; `unknown_char_policy='keep'` |
| `src/ocr_pipeline/segmenter.py` | `coalesce_mcq_segments`, `normalize_mcq_block_text` |
| `src/ocr_pipeline/quality.py` | Layer A admit + Layer B doc gate |
| `config/ocr_pipeline.yaml` | VLM + `mcq_stage3: sanitize` |
| `config/profiles/math_cp_p2.yaml` | Paper2 profile |
...
```powershell
uv run pytest tests/test_mcq_segment_merge.py tests/test_mcq_stage3_sanitize.py `
  tests/test_per_question_emit.py tests/test_mcq_coalesce.py -q
```
...
```yaml
# config/ocr_pipeline.yaml (relevant)
vlm:
