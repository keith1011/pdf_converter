...
if (-not (Test-Path (Join-Path $Repo 'homelab\ingest\ingest.py'))) {
...
& $py (Join-Path $Repo 'homelab\scripts\reader_neg_test.py')
...
if ($demo) {
  ...
  & $py (Join-Path $Repo 'homelab\ingest\ingest.py') --job-dir $demo.FullName
