"""UniMERNet formula adapter with no VLM fallback."""
...
DEFAULT_MODEL_ID = "opendatalab/UniMERNet"


def _load_unimernet_model(*, model_id: str, device: str) -> Any:
    ...
    try:
        return UnimernetModel(model_id, _device_=device)
    except Exception as exc:
        raise EngineError(
            f"UniMERNet could not load weights {model_id!r}. Ensure the model is "
...
class UnimernetFormulaEngine:
    ...
    def __init__(
        self, *, model_id: str = DEFAULT_MODEL_ID, device: str = "cuda"
    ) -> None:
    ...
    def _ensure_model(self) -> Any:
        if self._model is None:
            self._model = _load_unimernet_model(
